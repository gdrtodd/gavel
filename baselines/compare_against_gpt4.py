...
print(f"Initializing dummy archive...")
...
# Add the GPT-4 games to the archive as though they were generated by the main experiment
MODEL_NAME = "gpt-4o"
...
for section in GPT_SECTIONS:

    all_game_strs = np.load(open(f"./baselines/gpt_outputs/{MODEL_NAME}_outputs_{section}.npy", "rb"), allow_pickle=True)
    ...
    cleaned_game_strs = list(sorted(cleaned_game_strs))

    gpt_fitnesses = np.load(open(f"./baselines/gpt_outputs/{MODEL_NAME}_fitnesses_{section}.npy", "rb"), allow_pickle=True)
    gpt_evaluations = np.load(open(f"./baselines/gpt_outputs/{MODEL_NAME}_evaluations_{section}.npy", "rb"), allow_pickle=True)
    gpt_archive = {}

    for game, fitness, evaluation in tqdm(zip(cleaned_game_strs, gpt_fitnesses, gpt_evaluations),
                                    total=len(gpt_fitnesses),
                                    desc=f"Adding GPT-4 games to archive ({section})"):
        
        if fitness == -3 or np.isnan(fitness):
            continue

        cell = dummy_archive._get_cell(ArchiveGame(game, 0, {}, 0, [], 0, ""))
        if cell not in gpt_archive or fitness > gpt_archive[cell]:
            gpt_archive[cell] = fitness

    # Delete the "none" entry
    if "none" in gpt_archive:
        del gpt_archive["none"]

    # Compute stats for GPT-4
    qd_score = 0
    for key, fitness in gpt_archive.items():
        qd_score += (fitness - -2)
    qd_scores.append(qd_score)

    for threshold in FITNESS_THRESHOLDS:
        occupied = len([1 for fitness in gpt_archive.values() if fitness >= threshold])
        novel_occupied = len([1 for cell, fitness in gpt_archive.items() if fitness >= threshold and cell not in occupied_cells['any']])
